...
def result_txt_to_excel(filename, execl_name):
    ...
    sheet_dict['Precision'] = wb.create_sheet("精確度", 0)
    sheet_dict['Recall'] = wb.create_sheet("召回度", 0)
    sheet_dict['F1_score'] = wb.create_sheet("F1_score", 0)
    ...
    for line in content_list:
        ...
        if line[0] == '[':
            ...
            # line = line.split(',')
            value_list = list()
            for regex in [r'(?<=Acc=)\d+\.?\d*', r'(?<=Loss=)\d+\.?\d*', r'(?<=Precision=)\d+\.?\d*', r'(?<=Recall=)\d+\.?\d*', r'(?<=F1_score=)\d+\.?\d*']:
