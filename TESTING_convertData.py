...
def convertToUseful(data_arr):
    ...
    if bool(interpolation_dict):  # if interpolation_dict is not empty
        ...
        FishLog.writeLog(FishLog.formatLog(30, "TESTING_convertData.py", "line 221", "List the empty croods.", "[{}] empty croods: {}".format(_current_file, interpolation_dict)))
...
try:
    ...
    for dir in dirs:
        ...
        for file in csvfiles:
            _current_file = file
            print(file+".....")
            data_x = getCoords(CSVpath +'/' + file, False)
    ...
    if fail_times > 0:
        ...
        for path in failed_path_list:
            print("\t{}".format(path))
        FishLog.writeLog(FishLog.formatLog(30, "TESTING_convertData.py", "line 464", "List the fail files.", "failed files: {}".format(failed_path_list)))
    else:
        print("ALL SUCCESS")
    ...
    with open(temp_dir + '1000_all_result.txt', 'a') as f:
        f.write("[n_steps={:<2d} JUMP_N_FRAME={:<2d} is_normal={:<1d} scale_len={:<1d} is_translation={:<1d} is_rotate={:<1d}] len(train,test data)=({},{})\n".format(N_STEPS, JUMP_N_FRAME, IS_NORMAL, SCALE_LEN, IS_TRANSLATION, IS_ROTATE, total_actions['train'], total_actions['test']))

    return_code = 0
    if total_actions['train']<1000 or total_actions['test']<1:
        return_code = -1
    else:
    ...
    FishLog.writeLog(FishLog.formatException(e.args[0], extract_tb(tb)[0], "Convert data.", 'current file={}'.format(_current_file)))
else:
    # log
    FishLog.writeLog(FishLog.formatLog(20, "TESTING_convertData.py", "line 486", "Convert data.", "[len(train data)={} len(test_data)={} success_times={} / fail_times={}] [n_steps={:<2d} JUMP_N_FRAME={:<2d} is_normal={:<1d} scale_len={:<1d} is_rotate={:<1d}]".format(total_actions['train'], total_actions['test'], success_times, fail_times, N_STEPS, JUMP_N_FRAME, IS_NORMAL, SCALE_LEN, IS_ROTATE)))
